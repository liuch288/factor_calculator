...
在执行因子计算前，Agent 必须向用户确认以下信息（不可自行假设）：

1. **合约代码** — 如 TL2603、IF2403 等，用户不指定则必须询问
2. **日期/日期范围** — 单日模式或多日模式，用户不指定则必须询问
3. **Unit 参数** — 通过 `01_inspect_peu_params.py` 的方式查看签名后，向用户展示可选参数并确认
