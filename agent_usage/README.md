...
| `03_run_biquotepeu.py` | 自动计算前置依赖 MoSplitDMU，再计算 BiquotePEU | 无（脚本内自动处理依赖） |
| `04_run_klinedmu.py` | 计算 KlineDMU 1min 和 5min K 线 | 无 |
| `05_run_klinepatterndmu.py` | 计算 KlinePatternDMU K 线形态识别 | 需先运行 04（KlineDMU(1)） |
